...
<html lang="en" dir="ltr">
  ...
  $ic = filter_input(INPUT_POST,'st');

  $sql = "SELECT * FROM station where stat_id = $ic;";
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
  // output data of each row
  while($row = $result->fetch_assoc()) {
  echo "<tr><td>" . $row["cname"] . "</td><td>" . $row["stat_id"] . "</td><td>".$row["stat_loc"] . "</td><td>".$row["cap_l"] . "</td><td>".$row["oil_type"]."</td><td>".$row["tax"]. "</td><td>".$row["price"]. "</td></tr>";
  }
  echo "</table>";
  } else { echo "0 results"; }
  $conn->close();
